            ...
            if modelo != 'nan' and len(modelo) > 3:
                ...
                for n_val, col_idx in MAPA_N.items():
                    if col_idx < df_raw.shape[1]:
                        ...
                        if isinstance(val_bruto, str): val_limpo = val_bruto.replace(',', '.')
                        else: val_limpo = val_bruto
                        val = pd.to_numeric(val_limpo, errors='coerce')
                        capacidades[n_val] = val
                    else: capacidades[n_val] = None
...
def gerar_grade(h_ini_s, h_fim_s, tem_gin):
    ...
    marcos = sorted(list(set([h_ini_s, h_fim_s, almoco["ini"], almoco["fim"]] + [m for m in marcos_base if m_ini < p_min(m) < m_fim])))
    ...
    for i in range(len(marcos)-1):
        ...
        if m1 >= p_min(almoco["ini"]) and m2 <= p_min(almoco["fim"]):
            ...
            for po in pausas:
                ...
                if m1 < o_f and m2 > o_i: uteis -= (min(m2, o_f) - max(m1, o_i))
...
h_ini, h_fim = st.sidebar.text_input("Início Turno", "07:45"), st.sidebar.text_input("Fim Turno", "17:30")
    ...
    if st.button("🚀 GERAR PLANEJAMENTO"):
        if not df_input.empty:
            ...
            for _, row in df_input.iterrows():
                if row['Modelo']:
                    ...
                    if pd.isna(uh) or uh is None or uh <= 0:
                        st.error(f"⚠️ O modelo **{m_obj['ID']}** não possui Unidade/Hora para **N={n_dia}** na planilha.")
                        erro_n = True
                        break
            ...
            if not erro_n and fila:
                ...
                st.dataframe(pd.DataFrame(res).style.apply(style_almoco, axis=1), use_container_width=True)
else:
